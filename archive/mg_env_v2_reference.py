...
DEFAULT_PROJ_GAMMA = 0.5

# PROJ_DELTA — peak price bonus weight.
#   Extra reward for discharging during the most expensive periods.
#   The price score is defined as the ratio of the current sell price to the
#   average sell price over the day.  When price_score > 1.5 the battery
#   receives an additional bonus proportional to how much energy it discharges.
#   Recommended starting range: [0.5, 3.0]
DEFAULT_PROJ_DELTA = 2.0
# ─────────────────────────────────────────────────────────────────────────────
...
class MicroGridEnv(gym.Env):
    def __init__(self,**kwargs):
        ...
        self.proj_delta = kwargs.get("proj_delta", DEFAULT_PROJ_DELTA)
    ...
    def _projection_reward(self, sell_benefit, buy_cost, battery_cost, intended_action):
        ...
        # ── Profit term — the dominant reward signal ──────────────────────────
        price_norm = ((price_t - float(self.grid.sell_prices.min()))
                      / (float(self.grid.sell_prices.max())
                         - float(self.grid.sell_prices.min()) + 1e-6))
        sell_multiplier = 1.0 + price_norm  # ∈ [1.0, 2.0]
        profit = sell_multiplier * float(sell_benefit) - float(buy_cost) - 0.01 * float(battery_cost)

        # ── Executed action (post-clamping) ───────────────────────────────────
        executed_action = float(self.battery.energy_change)
        max_action      = float(max(abs(a) for a in ACTIONS))   # 80 kW

        # ── Gap penalty — small corrective nudge, not dominant ────────────────
        # Only penalises the agent when the physics had to override its action.
        # Kept deliberately small so the agent is not afraid to act.
        gap      = abs(float(intended_action) - executed_action)
        gap_norm = gap / max_action          # normalise to [0, 1]
        self.last_gap = gap_norm
        gap_penalty = 0.3 * gap_norm         # max ~0.3 — a nudge, not a wall

        # ── Price-aligned discharge bonus ─────────────────────────────────────
        # Directly reward selling when price is high.  This is the key lever
        # for encouraging aggressive discharge during expensive periods.
        if executed_action < 0:  # discharging
            discharge_bonus = p_norm * (abs(executed_action) / max_action)
        elif executed_action > 0:  # charging
            discharge_bonus = -(1.0 - p_norm) * (executed_action / max_action)
        else:
            discharge_bonus = 0.0

        # ── Peak price bonus — extra push during the most expensive hours ─────
        price_score = price_t / max(np.mean(self.grid.sell_prices), 1e-6)
        if executed_action < 0 and price_score > 1.2:
            peak_bonus = self.proj_delta * abs(executed_action) / max_action
        else:
            peak_bonus = 0.0

        # ── SOC drift penalty — gentle guard, not a hard wall ─────────────────
        # Prevents the battery from sitting at extreme SOC for long periods.
        # Much lighter than before so the agent freely arbitrages.
        current_soc = float(self.battery.SOC)
        soc_drift = max(current_soc - 0.8, 0.0) + max(0.2 - current_soc, 0.0)
        soc_penalty = 0.1 * soc_drift

        # ── Final reward ──────────────────────────────────────────────────────
        reward = profit - gap_penalty + discharge_bonus + peak_bonus - soc_penalty
