...
class QAgent:
    ...
    def __init__(
        ...
        freq_steps_update_target: int = 50,
        n_steps_warm_up_memory: int = 2000,
    ...
    def replay(self) -> None:
        ...
        for i in range(0, self.n_gradient_steps):
            ...
            with torch.no_grad():
                ...
                # .max() return max values in each row and indeces of max values, bcz in formula of TD we always take action giving max Q value
                next_q_values, _ = next_q_values.max(dim=1)

                # TD target
                target_q_values = (1 - done_batch) * next_q_values * self.discount_factor + reward_batch     # (1 - done_batch -> 0/1, here no target Q values for terminal state )
    ...
    def load_from_disk(cls, env: gym.Env, path: Path):
        ...
        # weights_only=False: this is a full pickled nn.Module (architecture +
        # weights), not just a state_dict, and PyTorch >=2.6 defaults to
        # weights_only=True which cannot unpickle it. Safe here since the
        # checkpoint is one we saved ourselves via `save_to_disk`.
        agent.q_net = torch.load(path / 'model', weights_only=False)
        agent.q_net.eval()
