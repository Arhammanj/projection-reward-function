...
# Load real wholesale prices from src/Prices.csv (falls back to synthetic
# data only if the real file is genuinely missing).
_csv_path = os.path.join(root_dir, 'Prices.csv')
if os.path.exists(_csv_path):
    DEFAULT_TOU_PRICE = np.genfromtxt(
        _csv_path, delimiter=';', skip_header=1, usecols=[1]
    ).astype(np.float32)
    # CSV prices are in pence(or cents)/kWh; scale to currency-unit/kWh
    DEFAULT_TOU_PRICE = DEFAULT_TOU_PRICE / 100.0
else:
    # Synthetic fallback — 48-point repeating daily ToU profile
    DEFAULT_TOU_PRICE = np.array([
        0.12, 0.11, 0.10, 0.09, 0.09, 0.10, 0.12, 0.15, 0.18, 0.21, 0.24, 0.26,
        0.28, 0.30, 0.31, 0.32, 0.31, 0.29, 0.27, 0.24, 0.20, 0.17, 0.14, 0.12,
    ] * 400, dtype=np.float32)  # repeated to cover 300 days * 24 hrs
...
class MicroGridEnv(gym.Env):
    ...
    def _build_state(self):
        ...
        # Wrap around the end of the price series so the 24h lookahead never
        # runs past the array bounds (real price data has no trailing buffer).
        _idx0 = self.day*self.iterations+self.time_step
        _idxs = np.arange(_idx0, _idx0+24) % len(self.grid.sell_prices)
        price_grid_sell = self.grid.sell_prices[_idxs]
        price_grid_sell = np.float32(price_grid_sell/max(self.grid.sell_prices))
...
if __name__ == '__main__':
    ...
    demo.seedy(1)
    s = demo.reset(day=0)
    demo.battery.current_capacity = 0.31   # near lower SOC wall
    ...
    print(f"  SOC before       : 0.31  (near lower wall 0.30)")
