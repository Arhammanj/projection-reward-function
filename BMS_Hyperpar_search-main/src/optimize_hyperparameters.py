...
def sample_hyper_parameters(
    ...
    discount_factor = trial.suggest_categorical("discount_factor", [0.99, 0.995])
    ...
    freq_steps_train = trial.suggest_categorical('freq_steps_train', [16, 32])
    ...
    n_steps_warm_up_memory = trial.suggest_categorical("n_steps_warm_up_memory", [5000, 10000])

    # how many consecutive gradient descent steps to perform when we update the main model parameters
    n_gradient_steps = trial.suggest_categorical("n_gradient_steps", [1, 4])
    ...
    epsilon_start = trial.suggest_categorical("epsilon_start", [1.0])

    # final value for the exploration rate
    epsilon_end = trial.suggest_uniform("epsilon_end", 0.1, 0.2)

    # for how many steps do we decrease epsilon from its starting value to
    # its final value `epsilon_end`
    steps_epsilon_decay = trial.suggest_categorical("steps_epsilon_decay", [int(1e5), int(2e5)])
